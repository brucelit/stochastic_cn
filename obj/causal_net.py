...
class CausalNet:
    """
    A Causal-net (cnet) implementation representing process models.
    ...
    def add_input_binding(self, activity: str, binding: Set[str]) -> None:
        """
        Add an input binding for an activity.
    ...
    def add_output_binding(self, activity: str, binding: Set[str]) -> None:
        """
        Add an output binding for an activity.
...
class Semantics:
    ...
    def __init__(self, causal_net: CausalNet):
        """
    ...
    def get_enabled_bindings(self, current_state: State):
        ...
        if current_state.is_empty():
            for output_binding in self.causal_net.output_bindings[self.causal_net.start_activity]:
                ...
                enabled_bindings.append(binding)
    ...
    def generate_all_valid_binding_sequences(self, max_depth: int = 10):
        ...
        def dfs(current_sequence, current_state, depth):
            ...
            for binding in enabled_bindings:
                ...
                # Continue the search
                dfs(new_sequence, new_state, depth + 1)
...
def print_cnet_info(cnet: CausalNet):
    """
    Print information about a Stochastic CausalNet object for debugging.

    Args:
        cnet: The CausalNet object to print
    """
    ...
    for activity in sorted(cnet.activities):
        if activity != cnet.start_activity:
            ...
            for binding in cnet.input_bindings[activity]:
                bindings.append(f"{list(binding)}")
